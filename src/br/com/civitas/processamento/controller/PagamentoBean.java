...
public class PagamentoBean extends AbstractCrudBean<Pagamento, PagamentoService> implements Serializable {
	...
	public void find(ActionEvent event) {
		...
		if (Objects.nonNull(getEntitySearch().getArquivo()) && Objects.nonNull(getCidade())) {
			...
			try {
				...
				list = service.getPagamentoPorArquivo(getEntitySearch().getArquivo(), cargosSelecionados,
						secretariasSelecionadas, setoresSelecionados,
			...
			FacesUtils.addWarnMessage("Selecione a Cidade, Ano e M�s para Consulta.");

		}

	}

	private void popularEventosSelecionados(List<Pagamento> list)
	...
	private void adicionarValorEventoPagamento(Pagamento pagamento, EventoPagamento ep,
			EventoPagamento eventoPagamentoZerado) {
	...
	private Map<String, Object> getMapValueFields(Pagamento pagamento)
		...
		if (Objects.nonNull(eventosSelecionados) && !eventosSelecionados.isEmpty()) {
			...
			for (EventoPagamento eventoPagamento : pagamento.getEventosPagamentoSelecionados()) {
				...
				pagamentosMap.put(eventoPagamento.getEvento().getNome(),
						String.format("%.2f", eventoPagamento.getValor()));
		...
		try {
			...
			for (String nomeColuna : pagamentosColumnsMap) {
				...
				for (Map<String, Object> mapa : pagamentosMap) {
					...
					row.createCell(numeroCelulaColuna)
							.setCellValue(getObject(nomeColuna, (HashMap<String, Object>) mapa));
